...
<head>
    ...
    <title>Liste des preneurs</title>
    ...
    <style>
        ...
        .page-subtitle { font-size: 1.1rem; opacity: .9; margin-top:.5rem; color:white!important; }

        .card-body { padding: 2.5rem; }

        .custom-alert {
            background: linear-gradient(135deg, #10b981, #059669);
            color: white; border-radius: 15px; padding: 1rem 1.5rem;
            margin: 1.5rem auto; width: 50%; text-align: center;
        ...
        .add-btn {
            ...
            color: white; padding: .75rem 2rem; border-radius: 50px; font-weight:600;
            text-decoration:none; display:inline-block; margin:1.5rem 0 2rem 2rem;
            box-shadow: 0 10px 30px rgba(14,165,233,0.3); transition: all .3s; position:relative; overflow:hidden;
        }
        ...
        .modern-table {
            ...
            border-radius:20px; overflow:hidden; box-shadow: 0 15px 40px rgba(15,23,42,0.1); margin:0;
        }
        .modern-table thead { background: linear-gradient(135deg, #0f172a 0%, #1e293b 50%, #0ea5e9 100%); }
        .modern-table thead th {
            color:white; padding:1.5rem 1rem; font-weight:600; text-align:center; font-size:.9rem; text-transform:uppercase;
        }
        ...
        .modern-table tbody tr:hover { background: linear-gradient(135deg, rgba(14,165,233,0.1), rgba(15,23,42,0.05)); transform: scale(1.01); box-shadow:0 5px 20px rgba(15,23,42,0.1); }
        .modern-table td { padding:1.25rem 1rem; text-align:center; border-bottom:1px solid rgba(226,232,240,0.8); font-weight:500; color:#334155; }

        .action-btn { padding:.5rem 1.25rem; border-radius:25px; font-weight:600; font-size:.85rem; display:inline-block; margin:0 .25rem; text-decoration:none; }
        ...
        .code-preneur {
            ...
            color:white; padding:.5rem 1rem; border-radius:50px; font-weight:600;
            font-family:'Courier New', monospace; display:inline-block; min-width:80px;
        }

        .preneur-name {
            font-weight:600; color:#0f172a; position:relative; padding-left:1.5rem;
        }
        .preneur-name::before { content:'🧑'; position:absolute; left:0; top:50%; transform:translateY(-50%); }
        ...
        @media (max-width:768px) {
            ...
            .modern-table thead th, .modern-table td { padding:.75rem .5rem; font-size:.8rem; }
            .action-btn { padding:.4rem .8rem; font-size:.75rem; margin:.1rem; }
            .code-preneur { padding:.3rem .8rem; font-size:.8rem; }
        }
    </style>
</head>
<body>
    ...
    <div id="d1">
        ...
        <div class="main-container">
            <div class="container">
                ...
                <div class="sites-card fade-in" role="region" aria-label="Liste des preneurs">
                    <div class="card-header">
                        <h1 class="page-title">📍 Liste des Preneurs</h1>
                        <p class="page-subtitle">Gérez tous vos preneurs en un seul endroit</p>
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <a href="/apreneur" class="add-btn">Ajouter un nouveau preneur</a>

                        <table class="modern-table" aria-describedby="preneur-table">
                            <thead>
                                <tr>
                                    <th>🆔 Matricule</th>
                                    <th>👤 Nom</th>
                            ...
                            <tbody>
                                @foreach($preneurs as $index => $preneur)
                                <tr class="table-row-animate" style="animation-delay: {{ $index * 0.06 }}s;">
                                    <td><span class="code-preneur">{{ $preneur->n_matricule }}</span></td>
                                    <td><span class="preneur-name">{{ $preneur->nom }}</span></td>
                                    <td>
                                        <a href="/updatepreneur?id={{ $preneur->id }}" class="action-btn btn-update">Modifier</a>
                                    </td>
                                    <td>
                                        <a href="/deletepreneur?id={{ $preneur->id }}" class="action-btn btn-delete delete-link">Supprimer</a>
                                    </td>
                                </tr>
                                @endforeach
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            deleteButtons.forEach(button => {
                button.addEventListener('click', function(e) {
                    e.preventDefault();
                    const matricule = this.closest('tr').querySelector('.code-preneur').textContent.trim();
                    if (confirm(`Êtes-vous sûr de vouloir supprimer le preneur "${matricule}" ?`)) {
                        // Si tu veux utiliser la méthode DELETE + CSRF, remplace par un formulaire POST.
